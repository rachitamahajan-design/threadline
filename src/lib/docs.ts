...
export async function summarizeDoc(db: DatabaseSync, title: string, text: string | null): Promise<string | null> {
  if (!hasOpenAI() || !text?.trim()) return null;
  try {
    // Closed loop like every other model workflow: recorded and metered.
    return await recordRun(
      db,
      { kind: "doc-summary", args: { title: title.slice(0, 120) } },
      async (budget) => {
        budget.spendUnits(1);
      ...
      (r) => ({ outcome: r ? "shipped" : "partial", steps: [], failure: null }),
    );
  } catch {
    return null; // a doc summary is a nicety — its failure is a recorded run, never a broken upload
  }
}
