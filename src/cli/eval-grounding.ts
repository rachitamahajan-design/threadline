 ...
 *
 * Recall is reported against the hand-labeled statements and themes in the fixtures.
 */
...
import { extractStatements } from "../pipeline/statements.js";
...
export function scoreNotes(fx: Fixture, notes: Notes, extra: { needsReview?: boolean; dropped?: number } = {}): Score {
  ...
  // Recall: is each hand-labeled statement present in some leaf, and each expected
  ...
  const statementHits = fx.expectedStatements.filter((f) => covered(f.text, leafBlob)).length;
  ...
  return {
    ...
    statementRecall: fx.expectedStatements.length ? statementHits / fx.expectedStatements.length : 1,
...
/** A labelled statement counts as covered when 60% of its content words show up. */
...
export async function evalNotes(fx: Fixture, live: boolean): Promise<Score> {
  ...
  const ctx = groundingContext({ segments: fx.segments, participants: fx.participants });
  const { set } = await extractStatements(fx.segments, ctx, { type: fx.type, participants: fx.participants });
  const out = await generateNotes(set.statements, ctx, { type: fx.type, participants: fx.participants });
  if (!out.value) {
    return { fixture: fx.id, leaves: 0, strict: 0, lexical: 0, statementRecall: 0, themeRecall: 0, needsReview: true, dropped: 0, failures: [] };
  }
  ...
  const out = await compose(
    {
      ...
      system: (f: string) => def.prompt.build({ statements: f, participants: fx.participants.join(", "), type: fx.type }),
...
async function main() {
  ...
  console.log("fixture         leaves  strict      lexical     stmts       themes      notes");
  ...
  for (const fx of fixtures) {
    ...
    console.log(
      `${fx.id.padEnd(15)} ${String(s.leaves).padStart(5)}  ` +
        ...
        `${bar(s.statementRecall)}${pct(s.statementRecall)}  ${bar(s.themeRecall)}${pct(s.themeRecall)}  ${flags}`,
    );
  ...
  console.log(
    `${"AVERAGE".padEnd(15)} ${String(scores.reduce((a, s) => a + s.leaves, 0)).padStart(5)}  ` +
      ...
      `${bar(avg((s) => s.statementRecall))}${pct(avg((s) => s.statementRecall))}  ${bar(avg((s) => s.themeRecall))}${pct(avg((s) => s.themeRecall))}`,
  );
